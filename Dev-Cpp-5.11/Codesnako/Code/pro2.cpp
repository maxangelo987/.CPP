...
void printrules(){
	cout<<"L = A DFA that will accept a string input. The string must be composed of parent's name(s) and a child's name."<<endl;
	cout<<"    In generating a parent's name, First letter must be any uppercase letter except 'O' and 'A',"<<endl;
	cout<<"    it must then be followed by lowercase letters (LC1), if an 'a' or 'o' is found"<<endl;
	cout<<"    and the next character is '+' or '=', then that makes it a parent's name."<<endl;
	cout<<"    If another parent is added, then a '+' must follow after the previous name."<<endl;
	cout<<"    If the 'a' or 'o', is followed by a '=', then the child's name must follow."<<endl;
	cout<<"    The child's name is generated in such a way that the first letter must be either 'O' or 'A',"<<endl;
	cout<<"    followed by any lowercase letter(s) in LC2 given that it is the equivalent letter from LC1."<<endl;
	cout<<"    A '$' must follow the child's name."<<endl;

	cout<<"LC1:"<<endl;
	cout<<"	b	c 	d 	e	 f	 g	 h	 i	 j	 k	 l	 m"<<endl;
	cout<<"LC2:"<<endl;
	cout<<"	n	p	q	r	 s	 t	 u	 v	 w	 x	 y	 z"<<endl<<endl<<endl;
}
void getinput(){
...
void out(){
	if(inv>0){
		cout<<endl<<"Invalid String Input";
	}
	else{
		dfa();
	}
...
void dfa(){  
	...
	outtextxy(150,600,str);	
	outtextxy(150,620,"Valid string input!");
